...
public class Craftconomy3TresorEconomy extends Provider<TresorEconomy, BukkitLoader> implements TresorEconomy  {

    private static final BigDecimal ZERO = BigDecimal.ZERO.setScale(128);

    public Craftconomy3TresorEconomy() {
        super(TresorEconomy.class);
    }
    ...
    @Override
    public BigDecimal getBalance(String playerName, String worldName) {
        if (!hasAccount(playerName)) {
            return ZERO;
        }
        return BigDecimal.valueOf(Common.getInstance().getAccountManager().getAccount(playerName, false)
                .getBalance(worldName, getCurrency(worldName).getName()));
    }

    @Override
    public BigDecimal getBalance(OfflinePlayer offlinePlayer, String worldName) {
        return getBalance(offlinePlayer.getUniqueId(), worldName);
    }

    @Override
    public BigDecimal getBalance(UUID playerId, String worldName) {
        Account account = getAccount(playerId);
        if (account == null) {
            return ZERO;
        }
    ...
    @Override
    public boolean has(OfflinePlayer offlinePlayer, BigDecimal amount) {
    ...
    @Override
    public boolean has(OfflinePlayer offlinePlayer, String worldName, BigDecimal amount) {
        return has(offlinePlayer.getUniqueId(), worldName, amount);
    }

    @Override
    public boolean has(UUID playerId, String worldName, BigDecimal amount) {
        Account account = getAccount(playerId);
        return account != null && account.hasEnough(amount.doubleValue(), worldName, getCurrency(worldName).getName());
    }

    @Override
    public EconomyResponse withdrawPlayer(String playerName, BigDecimal amount, String reason) {
        return withdrawPlayer(playerName, WorldGroupsManager.DEFAULT_GROUP_NAME, amount, reason);
    }

    @Override
    public EconomyResponse withdrawPlayer(OfflinePlayer offlinePlayer, BigDecimal amount, String reason) {
        return withdrawPlayer(offlinePlayer.getUniqueId(), WorldGroupsManager.DEFAULT_GROUP_NAME, amount, reason);
    }

    @Override
    public EconomyResponse withdrawPlayer(UUID playerId, BigDecimal amount, String reason) {
        return withdrawPlayer(playerId, WorldGroupsManager.DEFAULT_GROUP_NAME, amount, reason);
    }

    @Override
    public EconomyResponse withdrawPlayer(String playerName, String worldName, BigDecimal amount, String reason) {
        if (amount.compareTo(ZERO)  < 0) {
            return new EconomyResponse(ZERO, getBalance(playerName, worldName), EconomyResponse.ResponseType.FAILURE, "Cannot withdraw negative funds");
        }
        ...
        if (account.hasEnough(amount.doubleValue(), worldName, currency.getName())) {
            ...
            return new EconomyResponse(ZERO, getBalance(playerName, worldName), EconomyResponse.ResponseType.FAILURE, "Insufficient funds");
        }
    }

    @Override
    public EconomyResponse withdrawPlayer(OfflinePlayer offlinePlayer, String worldName, BigDecimal amount, String reason) {
        return withdrawPlayer(offlinePlayer.getUniqueId(), worldName, amount, reason);
    }

    @Override
    public EconomyResponse withdrawPlayer(UUID playerId, String worldName, BigDecimal amount, String reason) {
        if (amount.compareTo(ZERO)  < 0) {
            return new EconomyResponse(ZERO, getBalance(playerId, worldName), EconomyResponse.ResponseType.FAILURE, "Cannot withdraw negative funds");
        }
        ...
        if (account.hasEnough(amount.doubleValue(), worldName, currency.getName())) {
            ...
            return new EconomyResponse(ZERO, getBalance(playerId, worldName), EconomyResponse.ResponseType.FAILURE, "Insufficient funds");
        }
    }

    @Override
    public EconomyResponse depositPlayer(String playerName, BigDecimal amount, String reason) {
        return depositPlayer(playerName, WorldGroupsManager.DEFAULT_GROUP_NAME, amount, reason);
    }

    @Override
    public EconomyResponse depositPlayer(OfflinePlayer offlinePlayer, BigDecimal amount, String reason) {
        return depositPlayer(offlinePlayer, WorldGroupsManager.DEFAULT_GROUP_NAME, amount, reason);
    }

    @Override
    public EconomyResponse depositPlayer(UUID playerId, BigDecimal amount, String reason) {
        return depositPlayer(playerId, WorldGroupsManager.DEFAULT_GROUP_NAME, amount, reason);
    }

    @Override
    public EconomyResponse depositPlayer(String playerName, String worldName, BigDecimal amount, String reason) {
        if (amount.compareTo(ZERO)  < 0) {
            return new EconomyResponse(BigDecimal.ZERO, getBalance(playerName, worldName), EconomyResponse.ResponseType.FAILURE, "Cannot deposit negative funds");
        }
    ...
    @Override
    public EconomyResponse depositPlayer(OfflinePlayer offlinePlayer, String worldName, BigDecimal amount, String reason) {
        return depositPlayer(offlinePlayer.getUniqueId(), worldName, amount, reason);
    }

    @Override
    public EconomyResponse depositPlayer(UUID playerId, String worldName, BigDecimal amount, String reason) {
        if (amount.compareTo(ZERO) < 0) {
            return new EconomyResponse(ZERO, getBalance(playerId, worldName), EconomyResponse.ResponseType.FAILURE, "Cannot deposit negative funds");
        }
    ...
    @Override
    public EconomyResponse createBank(String name, UUID playerId) {
        return null;
    }
    ...
    @Override
    public EconomyResponse bankHas(String bankName, BigDecimal amount) {
        if (Common.getInstance().getAccountManager().exist(bankName, true)) {
    ...
    @Override
    public EconomyResponse bankWithdraw(String bankName, BigDecimal amount, String reason) {
        if (amount.compareTo(ZERO) < 0) {
    ...
    @Override
    public EconomyResponse bankDeposit(String bankName, BigDecimal amount, String reason) {
        if (amount.compareTo(ZERO) < 0) {
    ...
    @Override
    public EconomyResponse isBankOwner(String bankName, UUID playerId) {
    ...
    @Override
    public EconomyResponse isBankMember(String bankName, UUID playerId) {
